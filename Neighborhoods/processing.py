...
def card_rank(input_json):
    ...
    #Initialize all of the card_data dictionaries
    card_data = collections.defaultdict(list)

    #Reorganize data into output dictionary
    for idx in range(len(input_json['submissions'])):
        submission = input_json['submissions'][idx]

        #collect ranks for each card
        for card_val in submission:
            for card in card_val.keys():
                card_data[card].append(card_val[card])

    return {input_json['title']: card_data}
